...
---
# <a name="iteration-3--add-form-validation-c"></a>Iteración n.º 3: Agregar una validación de formulario (C#)
...
[![Cuadro de diálogo Nuevo proyecto](iteration-3-add-form-validation-cs/_static/image1.jpg)](iteration-3-add-form-validation-cs/_static/image1.png)
...
Observe la llamada al método auxiliar Html.ValidationSummary() que aparece inmediatamente encima del formulario HTML. Si hay mensajes de error de validación, este método muestra los mensajes de validación en una lista con viñetas.

Observe, además, las llamadas a Html.ValidationMessage() que aparecen junto a cada campo de formulario. La aplicación auxiliar ValidationMessage() muestra un mensaje de error de validación individual. En el caso del listado 1, se muestra un asterisco cuando hay un error de validación.

Por último, la aplicación auxiliar Html.TextBox() representa automáticamente una clase de hoja de estilos en cascada cuando hay un error de validación asociado a la propiedad mostrada por la aplicación auxiliar. La aplicación auxiliar Html.TextBox() representa una clase denominada **input-validation-error**.

Al crear una nueva aplicación ASP.NET MVC, se crea automáticamente una hoja de estilos denominada Site.css en la carpeta Contenido. Esta hoja de estilos contiene las siguientes definiciones para las clases CSS relacionadas con la aparición de mensajes de error de validación:
...
**Listado 2 - Controllers-ContactController.cs (Crear con validación)**
