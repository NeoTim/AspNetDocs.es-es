...
**Nota**: "sin especificar" solo está disponible en `system.web/httpCookies@sameSite` este momento. Esperamos agregar una sintaxis similar a los atributos de cookieSameSite mostrados anteriormente en futuras actualizaciones. La configuración `(SameSiteMode)(-1)` en el código sigue funcionando en las instancias de estas cookies. *
...
* Asegúrese de que *web.config* contiene lo siguiente:  <!-- review, I removed `debug="true"` -->
...
* Compruebe que los paquetes NuGet del proyecto tengan como destino la versión correcta de Framework. Puede comprobar la versión correcta del marco examinando el archivo de *packages.config* , por ejemplo:
  ...
  En el archivo de *packages.config* anterior, el `Microsoft.ApplicationInsights` paquete:
    * Está destinado a .NET 4.5.1.
    * Debe actualizar su `targetFramework` atributo a `net472` si existe un paquete actualizado destinado al destino de .NET Framework.
...
El cambio de comportamiento específico para .NET Framework es cómo la `SameSite` propiedad interpreta el `None` valor:

* Antes de que la revisión sea un valor de `None` pensado:
  * No emita el atributo.
* Después de la revisión:
  * `None`El valor significa "emitir el atributo con un valor de `None` ".
  * Un `SameSite` valor de `(SameSiteMode)(-1)` hace que el atributo no se emita.
...
* Especifica que las cookies que se validan explícitamente `SameSite=None` para habilitar la entrega entre sitios también deben marcarse como `Secure` .
...
* Las cookies de estado de sesión y autenticación de formularios ahora se escriben en la red como `Lax` en lugar de sin especificar.
  * Aunque la mayoría de las aplicaciones funcionan con `SameSite=Lax` cookies, las aplicaciones que se publican en sitios o aplicaciones que hacen uso de `iframe` pueden encontrar que el estado de sesión o las cookies de autorización de formularios no se usan según lo esperado. Para solucionarlo, cambie el `cookieSameSite` valor de la sección de configuración adecuada como se explicó anteriormente.
* Los httpCookies (que se establecen explícitamente `SameSite=None` en el código o la configuración ahora tienen ese valor escrito con la cookie, mientras que se omitió previamente. Esto puede producir problemas con exploradores más antiguos que solo admiten el estándar de borrador 2016.
  * Cuando el destino de los exploradores sea compatible con el estándar de borrador 2019 con las `SameSite=None` cookies, recuerde también marcarlos `Secure` o es posible que no se reconozcan.
...
* [SameSiteSupport.cs de C#](https://github.com/blowdart/AspNetSameSiteSamples/blob/master/SameSiteSupport.cs)
...
* [MVC de C#](xref:samesite/csMVC)
* [Formularios web de C#](xref:samesite/CSharpWebForms)
* [Formularios web de VB](xref:samesite/vbWF)
...
[Descargue](https://www.google.com/chrome/) una versión de Chrome que admita su nuevo atributo. En el momento de escribir este documento, la versión actual es Chrome 80. Chrome 80 necesita que la marca `chrome://flags/#same-site-by-default-cookies` esté habilitada para usar el nuevo comportamiento. También debe habilitar ( `chrome://flags/#cookies-without-same-site-must-be-secure` ) para probar el próximo comportamiento de las cookies que no tienen ningún atributo sameSite habilitado. Chrome 80 está en el destino para que el modificador trate las cookies sin el atributo como `SameSite=Lax` , aunque con un período de gracia agotado para ciertas solicitudes. Para deshabilitar el período de gracia tiempo, se puede iniciar Chrome 80 con el siguiente argumento de línea de comandos:
...
Actualice *web.config* para incluir los siguientes valores de configuración:
