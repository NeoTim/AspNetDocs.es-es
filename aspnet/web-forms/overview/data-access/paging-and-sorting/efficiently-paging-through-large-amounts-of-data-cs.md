...
title: Paginación eficaz a través de grandes cantidades de datos (C#) | Microsoft Docs
...
Debido a la facilidad de implementación, active una casilla y vuelva a realizarla. la paginación predeterminada es una opción atractiva. Sin embargo, su enfoque Naive en la recuperación de todos los registros lo convierte en una opción improbable al paginar a través de cantidades de datos suficientemente grandes o para sitios con muchos usuarios simultáneos. En tales circunstancias, debemos pasar a la paginación personalizada para proporcionar un sistema con capacidad de respuesta.
...
- **Índice de fila inicial** : índice de la primera fila de la página de datos que se va a mostrar; Este índice se puede calcular multiplicando el índice de la página por los registros que se van a mostrar por página y agregando uno. Por ejemplo, al paginar los registros 10 a la vez, en la primera página (cuyo índice de página es 0), el índice de la fila inicial es 0 \* 10 + 1 o 1; para la segunda página (cuyo índice de página es 1), el índice de la fila inicial es 1 \* 10 + 1 o 11.
...
Antes de examinar cómo recuperar el subconjunto preciso de los registros de la página que se está mostrando, eche un vistazo primero a cómo devolver el número total de registros que se van a paginar. Esta información es necesaria para configurar correctamente la interfaz de usuario de paginación. El número total de registros devueltos por una consulta SQL determinada se puede obtener mediante la [ `COUNT` función de agregado](https://msdn.microsoft.com/library/ms175997.aspx). Por ejemplo, para determinar el número total de registros de la `Products` tabla, podemos usar la siguiente consulta:

[!code-sql[Main](efficiently-paging-through-large-amounts-of-data-cs/samples/sample1.sql)]

Permite agregar un método a la capa DAL que devuelve esta información. En concreto, vamos a crear un método DAL denominado `TotalNumberOfProducts()` que ejecute la `SELECT` instrucción mostrada anteriormente.

Para empezar, abra el `Northwind.xsd` archivo de conjunto de archivos con tipo en la `App_Code/DAL` carpeta. A continuación, haga clic con el botón derecho en el `ProductsTableAdapter` en el diseñador y elija Agregar consulta. Como hemos aprendido en los tutoriales anteriores, esto nos permitirá agregar un nuevo método a la capa DAL que, cuando se invoca, ejecutará una instrucción SQL o un procedimiento almacenado determinado. Al igual que con nuestros métodos de TableAdapter en los tutoriales anteriores, para esta opción se opta por usar una instrucción SQL ad hoc.
...
En la pantalla siguiente, se puede especificar el tipo de consulta que se va a crear. Dado que esta consulta devolverá un único valor escalar, el número total de registros de la `Products` tabla elige el `SELECT` que devuelve una opción de valor único.
...
Por último, especifique el nombre del método. Como ya se ha mencionado, permita usar `TotalNumberOfProducts` .
...
Después de hacer clic en finalizar, el asistente agregará el `TotalNumberOfProducts` método a la capa dal. Los métodos que devuelven valores escalares en la capa DAL devuelven tipos que aceptan valores NULL, en caso de que el resultado de la consulta SQL sea `NULL` . `COUNT`La consulta, sin embargo, siempre devolverá un valor que no es de `NULL` ; sin embargo, el método Dal devuelve un entero que acepta valores NULL.

Además del método DAL, también necesitamos un método en la capa BLL. Abra el `ProductsBLL` archivo de clase y agregue un `TotalNumberOfProducts` método que simplemente llame al método Dal s `TotalNumberOfProducts` :

[!code-csharp[Main](efficiently-paging-through-large-amounts-of-data-cs/samples/sample2.cs)]

El método DAL s `TotalNumberOfProducts` devuelve un entero que acepta valores NULL; sin embargo, hemos creado el `ProductsBLL` método Class s `TotalNumberOfProducts` para que devuelva un entero estándar. Por lo tanto, es necesario que el `ProductsBLL` método Class s `TotalNumberOfProducts` devuelva la parte del valor del entero que acepta valores NULL que devuelve el método Dal s `TotalNumberOfProducts` . La llamada a `GetValueOrDefault()` devuelve el valor del entero que acepta valores NULL, si existe; si el entero que acepta valores NULL es `null` , sin embargo, devuelve el valor entero predeterminado, 0.
...
Esto no es un desafío si ya existe una columna en la tabla de base de datos que actúa como índice de fila. A primera vista, es posible que creemos que el `Products` campo tabla s `ProductID` bastaría, ya que el primer producto tiene `ProductID` 1, el segundo a 2, etc. Sin embargo, la eliminación de un producto deja un hueco en la secuencia, anulando este enfoque.
...
- **Uso de SQL Server 2005 s `ROW_NUMBER()` Palabra clave** New en SQL Server 2005, la `ROW_NUMBER()` palabra clave asocia una clasificación con cada registro devuelto según el orden. Esta clasificación se puede usar como índice de fila para cada fila.
- **Usar una variable de tabla `SET ROWCOUNT` y** Se puede utilizar SQL Server [ `SET ROWCOUNT` instrucción](https://msdn.microsoft.com/library/ms188774.aspx) para especificar el número total de registros que debe procesar una consulta antes de finalizar. [las variables de tabla](http://www.sqlteam.com/item.asp?ItemID=9454) son variables de T-SQL locales que pueden contener datos tabulares, similar a [las tablas temporales](http://www.sqlteam.com/item.asp?ItemID=2029). Este enfoque funciona igual de bien con Microsoft SQL Server 2005 y SQL Server 2000 (mientras que el `ROW_NUMBER()` enfoque solo funciona con SQL Server 2005).  
  
  La idea aquí es crear una variable de tabla que tenga una `IDENTITY` columna y columnas para las claves principales de la tabla cuyos datos se van a paginar. Después, el contenido de la tabla cuyos datos se están paginando se vuelca en la variable de tabla, con lo que se asocia un índice de fila secuencial (a través de la `IDENTITY` columna) para cada registro de la tabla. Una vez que se ha rellenado la variable de tabla, `SELECT` se puede ejecutar una instrucción en la variable de tabla, unida a la tabla subyacente, para extraer los registros concretos. La `SET ROWCOUNT` instrucción se utiliza para limitar de forma inteligente el número de registros que se deben volcar en la variable de tabla.  
  
  La eficacia de este enfoque se basa en el número de página que se solicita, ya que el valor `SET ROWCOUNT` se asigna al valor de inicio de índice de fila más las filas máximas. Al paginar a través de páginas de número bajo, como las primeras páginas de datos, este enfoque es muy eficaz. Sin embargo, exhibe un rendimiento similar a la paginación predeterminada al recuperar una página cerca del final.

En este tutorial se implementa la paginación personalizada mediante la `ROW_NUMBER()` palabra clave. Para obtener más información sobre el uso de la variable de tabla y la `SET ROWCOUNT` técnica, vea [un método más eficaz para la paginación a través de grandes conjuntos de resultados](http://www.4guysfromrolla.com/webtech/042606-1.shtml).

La `ROW_NUMBER()` palabra clave asoció una clasificación con cada registro devuelto a través de una ordenación determinada con la siguiente sintaxis:
...
Al clasificar los resultados por la `ORDER BY` columna especificada en la `OVER` cláusula ( `UnitPrice` en el ejemplo anterior), SQL Server debe ordenar los resultados. Se trata de una operación rápida si hay un índice clúster en las columnas en las que se están ordenando los resultados, o si hay un índice de cobertura, pero puede ser más costoso en caso contrario. Para ayudar a mejorar el rendimiento de las consultas suficientemente grandes, considere la posibilidad de agregar un índice no agrupado para la columna por la que se ordenan los resultados. Consulte [funciones de categoría y rendimiento en SQL Server 2005](http://www.sql-server-performance.com/ak_ranking_functions.asp) para obtener una visión más detallada de las consideraciones de rendimiento.

La información de clasificación que devuelve `ROW_NUMBER()` no se puede usar directamente en la `WHERE` cláusula. Sin embargo, se puede usar una tabla derivada para devolver el `ROW_NUMBER()` resultado, que puede aparecer en la `WHERE` cláusula. Por ejemplo, la consulta siguiente usa una tabla derivada para devolver las columnas ProductName y UnitPrice, junto con el `ROW_NUMBER()` resultado, y, a continuación, utiliza una `WHERE` cláusula para devolver solo los productos cuyo rango de precio está entre 11 y 20:
...
> [!NOTE]
> Como veremos más adelante en este tutorial, el *`StartRowIndex`* proporcionado por ObjectDataSource se indexa a partir de cero, mientras que el `ROW_NUMBER()` valor devuelto por SQL Server 2005 se indiza a partir de 1. Por lo tanto, la `WHERE` cláusula devuelve los registros donde `PriceRank` es estrictamente mayor que *`StartRowIndex`* y menor o igual que *`StartRowIndex`*  +  *`MaximumRows`* .

Ahora que se ha explicado cómo `ROW_NUMBER()` se puede usar para recuperar una página de datos determinada según los valores de índice de la fila inicial y filas máximas, ahora es necesario implementar esta lógica como métodos en Dal y BLL.
...
En la sección anterior, creamos el método DAL como una instrucción SQL ad hoc. Desafortunadamente, el analizador de T-SQL de Visual Studio que usa el Asistente de TableAdapter no es como la `OVER` sintaxis usada por la `ROW_NUMBER()` función. Por lo tanto, es necesario crear este método DAL como un procedimiento almacenado. Seleccione el Explorador de servidores en el menú Ver (o presione Ctrl + Alt + S) y expanda el `NORTHWND.MDF` nodo. Para agregar un nuevo procedimiento almacenado, haga clic con el botón secundario en el nodo procedimientos almacenados y elija Agregar un nuevo procedimiento almacenado (vea la figura 6).
...
Este procedimiento almacenado debe aceptar dos parámetros de entrada enteros: `@startRowIndex` y `@maximumRows` y usar la `ROW_NUMBER()` función ordenada por el `ProductName` campo, devolviendo solo las filas mayores que el `@startRowIndex` valor de especificado y menor o igual que `@startRowIndex`  +  `@maximumRow` s. Escriba el siguiente script en el nuevo procedimiento almacenado y, a continuación, haga clic en el icono guardar para agregar el procedimiento almacenado a la base de datos.
...
![Escriba un valor para los @startRowIndex @maximumRows parámetros y](efficiently-paging-through-large-amounts-of-data-cs/_static/image7.png)

<strong>Figura 7</strong>: especificación de un valor para @startRowIndex los @maximumRows parámetros y

Después de elegir estos valores de parámetros de entrada, en la ventana de salida se mostrarán los resultados. En la figura 8 se muestran los resultados al pasar 10 para `@startRowIndex` los `@maximumRows` parámetros y.
...
A continuación, se le pedirá que seleccione el procedimiento almacenado que se va a invocar. Seleccione el `GetProductsPaged` procedimiento almacenado en la lista desplegable.
...
Puede usar cualquier nombre para los parámetros de entrada del método BLL, pero, como veremos en breve, eligiendo usar `startRowIndex` y `maximumRows` nos ahorra de un bit de trabajo adicional al configurar un ObjectDataSource para usar este método.
...
Con los métodos BLL y DAL para tener acceso a un subconjunto de registros determinado, se vuelve a preparar la creación de un control GridView que recorre los registros subyacentes mediante la paginación personalizada. Para empezar, abra la `EfficientPaging.aspx` página en la `PagingAndSorting` carpeta, agregue un control GridView a la página y configúrelo para que use un nuevo control ObjectDataSource. En los tutoriales anteriores, a menudo tuvimos configurado ObjectDataSource para usar el `ProductsBLL` método Class s `GetProducts` . Sin embargo, esta vez queremos usar el método en `GetProductsPaged` su lugar, ya que el `GetProducts` método devuelve *todos* los productos de la base de datos, mientras que `GetProductsPaged` devuelve solo un subconjunto determinado de registros.
...
A continuación, el Asistente para ObjectDataSource nos solicita los orígenes de los `GetProductsPaged` valores de los parámetros de método `startRowIndex` y de `maximumRows` entrada. Los parámetros de entrada se establecerán realmente mediante GridView automáticamente, por lo que simplemente deje el origen establecido en ninguno y haga clic en finalizar.
...
Después de completar el Asistente de ObjectDataSource, GridView contendrá un BoundField o CheckBoxField para cada uno de los campos de datos de producto. No dude en adaptar la apariencia de GridView como considere oportuno. He optado por mostrar solo los `ProductName` , `CategoryName` , `SupplierName` , `QuantityPerUnit` y `UnitPrice` BoundFields. Además, configure GridView para que admita la paginación. para ello, active la casilla habilitar paginación en su etiqueta inteligente. Después de estos cambios, el marcado declarativo de GridView y ObjectDataSource debe ser similar al siguiente:
...
Falta GridView porque ObjectDataSource está usando actualmente 0 como valores para los `GetProductsPaged` `startRowIndex` `maximumRows` parámetros de entrada y. Por lo tanto, la consulta SQL resultante no devuelve ningún registro y, por tanto, no se muestra GridView.
...
1. **Establezca la propiedad ObjectDataSource s `EnablePaging` en `true` ** esto indica al ObjectDataSource que debe pasar a los `SelectMethod` dos parámetros adicionales: uno para especificar el índice de la fila inicial ( [`StartRowIndexParameterName`](https://msdn.microsoft.com/library/system.web.ui.webcontrols.objectdatasource.startrowindexparametername.aspx) ) y otro para especificar el número máximo de filas ( [`MaximumRowsParameterName`](https://msdn.microsoft.com/library/system.web.ui.webcontrols.objectdatasource.maximumrowsparametername.aspx) ).
2. **Establezca las propiedades de `StartRowIndexParameterName` ObjectDataSource `MaximumRowsParameterName` y** en consecuencia `StartRowIndexParameterName` . las `MaximumRowsParameterName` propiedades y indican los nombres de los parámetros de entrada que se pasan al `SelectMethod` para la paginación personalizada. De forma predeterminada, estos nombres de parámetro son `startIndexRow` y `maximumRows` , que es el motivo por el que, al crear el `GetProductsPaged` método en el BLL, usé estos valores para los parámetros de entrada. Si decide usar diferentes nombres de parámetro para el método BLL s `GetProductsPaged` como `startIndex` y `maxRows` , por ejemplo, debe establecer las propiedades de ObjectDataSource `StartRowIndexParameterName` y en `MaximumRowsParameterName` consecuencia (como startIndex para `StartRowIndexParameterName` y maxRows para `MaximumRowsParameterName` ).
3. **Establezca la [ `SelectCountMethod` propiedad](https://msdn.microsoft.com/library/system.web.ui.webcontrols.objectdatasource.selectcountmethod(VS.80).aspx) ObjectDataSource s en el nombre del método que devuelve el número total de registros que se van a paginar mediante ( `TotalNumberOfProducts` )** Recuerde que el `ProductsBLL` método Class s `TotalNumberOfProducts` devuelve el número total de registros que se van a paginar mediante un método dal que ejecuta una `SELECT COUNT(*) FROM Products` consulta. Esta información es necesaria para que ObjectDataSource pueda representar correctamente la interfaz de paginación.
4. **Quitar los `startRowIndex` `maximumRows` `<asp:Parameter>` elementos y del marcado declarativo de ObjectDataSource s** al configurar ObjectDataSource mediante el asistente, Visual Studio agrega automáticamente dos `<asp:Parameter>` elementos para los `GetProductsPaged` parámetros de entrada del método. Al establecer `EnablePaging` en `true` , estos parámetros se pasarán automáticamente; si también aparecen en la sintaxis declarativa, ObjectDataSource intentará pasar *cuatro* parámetros al `GetProductsPaged` método y dos parámetros al `TotalNumberOfProducts` método. Si olvida quitar estos `<asp:Parameter>` elementos, al visitar la página a través de un explorador, obtendrá un mensaje de error similar a: *ObjectDataSource ' ObjectDataSource1 ' no encontró un método no genérico ' TotalNumberOfProducts ' que tenga parámetros: StartRowIndex, maximumRows*.
...
Tenga en cuenta `EnablePaging` que `SelectCountMethod` se han establecido las propiedades y y `<asp:Parameter>` se han quitado los elementos. En la figura 16 se muestra una captura de pantalla de la ventana Propiedades una vez realizados estos cambios.
...
> [!NOTE]
> Con la paginación personalizada, el valor de recuento de páginas que devuelve ObjectDataSource `SelectCountMethod` se almacena en el estado de vista de GridView. Otras variables de GridView `PageIndex` la `EditIndex` colección,, `SelectedIndex` , `DataKeys` , etc., se almacenan en el *Estado del control*, que se conserva independientemente del valor de la `EnableViewState` propiedad de GridView. Dado que el `PageCount` valor se conserva en los postbacks mediante el estado de vista, cuando se usa una interfaz de paginación que incluye un vínculo para ir a la última página, es imperativo que el estado de vista de GridView esté habilitado. (Si la interfaz de paginación no incluye un vínculo directo a la última página, puede deshabilitar el estado de vista).

Al hacer clic en el vínculo de la última página se produce un postback y se indica a GridView que actualice su `PageIndex` propiedad. Si se hace clic en el vínculo de la última página, GridView asigna su `PageIndex` propiedad a un valor inferior a su `PageCount` propiedad. Con el estado de vista deshabilitado, el `PageCount` valor se pierde en los postbacks y `PageIndex` se asigna el valor entero máximo en su lugar. A continuación, el control GridView intenta determinar el índice de la fila inicial multiplicando las `PageSize` `PageCount` propiedades y. Esto da como resultado una `OverflowException` , ya que el producto supera el tamaño máximo de entero permitido.
...
La ordenación solo se aplica a la página de datos actual, ya que la ordenación se produce después de que los datos se han recuperado del método BLL s `GetProductsPaged` y este método solo devuelve los registros de la página específica. Para implementar la ordenación correctamente, es necesario pasar la expresión de ordenación al `GetProductsPaged` método para que los datos se puedan clasificar adecuadamente antes de devolver la página de datos específica. Veremos cómo hacerlo en el siguiente tutorial.

## <a name="implementing-custom-paging-and-deleting"></a>Implementar la paginación y eliminación personalizadas

Si habilita la eliminación de la funcionalidad en un control GridView cuyos datos se paginan mediante técnicas de paginación personalizadas, observará que, al eliminar el último registro de la última página, el control GridView desaparece en lugar de reducir de forma adecuada los objetos GridView `PageIndex` . Para reproducir este error, habilite la eliminación del tutorial que acabamos de crear. Vaya a la última página (página 9), donde debería ver un solo producto, ya que estamos paginando a través de 81 productos, 10 productos a la vez. Elimine este producto.
...
2. Obtiene los registros adecuados que se van a mostrar para el y el especificados. `PageIndex``PageSize`
3. Asegúrese de que el no `PageIndex` supera el número de páginas de datos del origen de datos; si es así, disminuya automáticamente la propiedad GridView s. `PageIndex`
4. Enlazar la página de datos adecuada a GridView usando los registros obtenidos en el paso 2

El problema se debe al hecho de que en el paso 2, el que se `PageIndex` usa al capturar los registros que se van a mostrar sigue siendo el `PageIndex` de la última página cuyo único registro se ha eliminado. Por lo tanto, en el paso 2, no se devuelve *ningún* registro, ya que la última página de datos ya no contiene ningún registro. A continuación, en el paso 3, el control GridView se entiende que su `PageIndex` propiedad es mayor que el número total de páginas del origen de datos (desde que se eliminó el último registro de la última página) y, por tanto, disminuye su `PageIndex` propiedad. En el paso 4, el control GridView intenta enlazarse a los datos recuperados en el paso 2; sin embargo, en el paso 2 no se devolvieron registros, por lo que se produce un control GridView vacío. Con la paginación predeterminada, este problema no tiene la superficie t porque en el paso 2 se recuperan *todos* los registros del origen de datos.

Para solucionar este comportamiento tenemos dos opciones. El primero es crear un controlador de eventos para el controlador de `RowDeleted` eventos GridView s que determina cuántos registros se han mostrado en la página que se acaba de eliminar. Si solo había un registro, el registro que se acaba de eliminar debe haber sido el último y es necesario reducir los objetos GridView `PageIndex` . Por supuesto, solo queremos actualizar `PageIndex` si la operación de eliminación se realizó correctamente, lo que se puede determinar asegurándose de que la `e.Exception` propiedad es `null` .
...
Una solución alternativa consiste en crear un controlador de eventos para el evento ObjectDataSource s `RowDeleted` y establecer la `AffectedRows` propiedad en un valor de 1. Después de eliminar el registro del paso 1 (pero antes de volver a recuperar los datos en el paso 2), GridView actualiza su `PageIndex` propiedad si una o más filas se vieron afectadas por la operación. Sin embargo, `AffectedRows` ObjectDataSource no establece la propiedad y, por lo tanto, se omite este paso. Una manera de ejecutar este paso consiste en establecer manualmente la `AffectedRows` propiedad si la operación de eliminación se completa correctamente. Esto puede realizarse mediante código como el siguiente:

[!code-csharp[Main](efficiently-paging-through-large-amounts-of-data-cs/samples/sample12.cs)]

El código de ambos controladores de eventos se puede encontrar en la clase de código subyacente del `EfficientPaging.aspx` ejemplo.
