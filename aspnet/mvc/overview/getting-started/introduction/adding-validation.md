...
En esta sección, agregará la lógica de validación al `Movie` modelo y se asegurará de que se apliquen las reglas de validación cada vez que un usuario intente crear o editar una película mediante la aplicación.
...
Empezará agregando una lógica de validación a la `Movie` clase.

Abra el archivo *Movie.cs*. Observe que el [`System.ComponentModel.DataAnnotations`](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.aspx) espacio de nombres no contiene `System.Web` . DataAnnotations proporciona un conjunto integrado de atributos de validación que puede aplicar mediante declaración a cualquier clase o propiedad. (También contiene atributos de formato como [DataType](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.datatype.aspx) que ayudan a dar formato y no proporcionan ninguna validación).

Ahora actualice la `Movie` clase para aprovechar los [`Required`](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.requiredattribute.aspx) atributos integrados, [`StringLength`](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.stringlengthattribute.aspx) , [RegularExpression](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.regularexpressionattribute.aspx)y de [`Range`](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.rangeattribute.aspx) validación. Reemplace la `Movie` clase por lo siguiente:
...
Cuando finaliza este comando, Visual Studio abre el archivo de clase que define la nueva `DbMigration` clase derivada con el nombre especificado ( `DataAnnotations` ) y, en el `Up` método, puede ver el código que actualiza las restricciones de esquema:

[!code-csharp[Main](adding-validation/samples/sample3.cs)]

El `Genre` campo ya no acepta valores NULL (es decir, debe escribir un valor). El `Rating` campo tiene una longitud máxima de 5 y `Title` tiene una longitud máxima de 60. La longitud mínima de 3 en `Title` y el intervalo en `Price` no creó cambios de esquema.
...
Tal vez se pregunte cómo se generó la validación de la interfaz de usuario sin actualizar el código en el controlador o las vistas. La siguiente lista muestra el aspecto de los `Create` métodos de la `MovieController` clase. No se modifican de la forma en que se crearon anteriormente en este tutorial.

[!code-csharp[Main](adding-validation/samples/sample5.cs)]

El primer método de acción `Create` (HTTP GET) muestra el formulario de creación inicial. La segunda versión (`[HttpPost]`) controla el envío de formulario. El segundo `Create` método (la `HttpPost` versión) comprueba `ModelState.IsValid` si la película tiene errores de validación. Al obtener esta propiedad se evalúan todos los atributos de validación que se han aplicado al objeto. Si el objeto tiene errores de validación, el `Create` método vuelve a mostrar el formulario. Si no hay ningún error, el método guarda la nueva película en la base de datos. En nuestro ejemplo de película, **el formulario no se publica en el servidor cuando se detectan errores de validación en el lado cliente;** `Create` **nunca se llama**al segundo método. Si deshabilita JavaScript en el explorador, la validación del cliente está deshabilitada y el `Create` método http post obtiene la `ModelState.IsValid` comprobación de si la película tiene errores de validación.
...
Observe cómo el código utiliza una `Html.EditorFor` aplicación auxiliar para generar el `<input>` elemento para cada `Movie` propiedad. Junto a esta aplicación auxiliar es una llamada al `Html.ValidationMessageFor` método auxiliar. Estos dos métodos auxiliares funcionan con el objeto de modelo que pasa el controlador a la vista (en este caso, un `Movie` objeto). Buscan automáticamente los atributos de validación especificados en el modelo y muestran los mensajes de error según corresponda.
